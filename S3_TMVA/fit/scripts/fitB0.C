...
void fitB0()
{
    ...
    RooRealVar *BDTval = new RooRealVar("BDT", "BDTval", -1, 1);

    // --- unbin data
    // RooDataSet *data = new RooDataSet("data","data", t_data_cut, RooArgSet(*PVDFit_DmPpPmpi_M));

    RooDataSet *data = new RooDataSet("data","data",RooArgSet(*PVDFit_DmPpPmpi_M, *PVFit_Dm_M, *BDTval), Import(*t_data), Cut("BDT > -0.14"));

    // --- Build Gaussian PDFs ---
    // RooRealVar* mean   = new RooRealVar("mean",   "mean  of gauss",  5279, 5265, 5295);
    // RooRealVar* sigmaGauss = new RooRealVar("sigmaGauss", "width of gauss1", 10.,    0, 30);
    ...
    // RooGaussian* sigpdf = new RooGaussian("sigpdf","Signal component 1",  *PVDFit_DmPpPmpi_M, *mean, *sigmaGauss) ;

    // --- Build signal pdf ---
    RooRealVar *meanGauss   = new RooRealVar("mean",   "mean  of gauss",  5279., 5265, 5295);
    RooRealVar *sigmaGauss = new RooRealVar("sigmaGauss", "width of gauss1", 10.,    0, 30);
    RooRealVar *gammaBW = new RooRealVar("sigma", "width of the BW", 10., 0, 30);

    //RooGaussian* Gausspdf = new RooGaussian("gausspdf","Signal component 1",  *PVDFit_DmPpPmpi_M, *mean, *sigmaGauss) ;
    ...
    //RooBreitWigner *BWpdf = new RooBreitWigner("BW pdf", "Signal Component 2", *PVDFit_DmPpPmpi_M, *mean, *sigma);

    // RooFFTConvPdf sigpdf("sigpdf", "Signal Component 1*2", *PVDFit_DmPpPmpi_M, *Gausspdf, *BWpdf);
    RooVoigtian sigpdf("Bsig", "Bsig", *PVDFit_DmPpPmpi_M, *meanGauss, *gammaBW, *sigmaGauss);
    ...
    // --- Total PDFs ---
    RooRealVar nsig("nsig", "", 4.0e04, 0, 1e5);
    RooRealVar nbkg("nbkg", "", 5e04, 0, 1e6);
    ...
    // Double_t v_s0 = sigmaGauss->getVal(), e_s0 = sigmaGauss->getError();
    ...
    // Double_t mean_max = Mean + 3.0*Sigma;
    // Double_t mean_min = Mean - 3.0*Sigma;
    // cout << "Mean(+/- 3.0*Sigma) = " << mean_min << ", " << mean_max << endl;
    ...
    // Calculation of the integrals for Signal and Background events in SR and SB
    PVDFit_DmPpPmpi_M->setRange("signal", mean_min, mean_max);
    PVDFit_DmPpPmpi_M->setRange("bkg", 5350, 5400);
    ...
    RooAbsReal* int_bkg_sb_region = bkgpdf->createIntegral(*PVDFit_DmPpPmpi_M,NormSet(*PVDFit_DmPpPmpi_M),Range("bkg"));
    cout << "int BKG sb = " << int_bkg_sb_region->getVal() << endl;
    ...
    line.SetLineStyle(2);
    line.DrawLine(mean_min,0,mean_min,max);
    line.DrawLine(mean_max,0,mean_max,max);
